...
namespace UnLua
{
    ...
    TSharedPtr<ITypeInterface> FPropertyRegistry::GetFieldProperty(UField* Field)
    {
        ...
        {
#if UE_VERSION_OLDER_THAN(5, 1, 0)
            Property = new FEnumProperty(PropertyCollector, NAME_None, RF_Transient, 0, CPF_HasGetValueTypeHash, Enum);
#else
            constexpr auto Params = UECodeGen_Private::FEnumPropertyParams
            {
                nullptr,
                nullptr,
                CPF_HasGetValueTypeHash,
            ...
            Property = new FEnumProperty(PropertyCollector, Params);
#endif
            const auto UnderlyingProperty = new FByteProperty(Property, TEXT("UnderlyingType"), RF_Transient);
